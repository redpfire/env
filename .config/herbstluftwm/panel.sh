...
y=${geometry[3]}
y=$(($y - $panel_height))
# font="scientifica:pixelsize=12:antialias=false"
font="Iosevka Custom:pixelsize=10:antialias=true:autohint=true"
bgcolor='#000000'
selbg='#000000'
selfg='#ffffff'
...
hc pad $monitor 0 0 $panel_height

{
    ...
    #mpc idleloop player &
    conky -c $(dirname $0)/conky.lua &
    while true ; do
        # "date" output is checked once a second, but an event is only
        # generated if the output changed compared to the previous run.
        date +$'date\t^fg(#efefef)%a %d/%m %H:%M'
        sleep 1 || break
    ...
    while true ; do
        ...
        # small adjustments
        right="$separator^bg() $conky $date $separator  "
